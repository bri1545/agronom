...
Отвечай КРАТКО, без лишних объяснений. Формат процентов: "Пшеница: 30%, Ячмень: 25%" и т.д.`;
    ...
    const dailyFeed = percentageMatches.slice(0, 7).map(match => {
      ...
      return {
        ...
        amountPerAnimal: `${Math.round(percentage * 0.15)} кг/день`,
      };
    });

    if (dailyFeed.length === 0) {
      dailyFeed.push(
        { ingredient: "Пшеница", percentage: 30, amountPerAnimal: "4.5 кг/день" },
        { ingredient: "Ячмень", percentage: 25, amountPerAnimal: "3.8 кг/день" },
        { ingredient: "Кукуруза", percentage: 20, amountPerAnimal: "3 кг/день" },
        { ingredient: "Сено", percentage: 15, amountPerAnimal: "2.3 кг/день" },
        { ingredient: "Витамины", percentage: 10, amountPerAnimal: "1.5 кг/день" }
      );
    }
